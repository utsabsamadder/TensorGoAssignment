...
# Force TensorFlow to use CPU by disabling GPU
os.environ["CUDA_VISIBLE_DEVICES"] = "-1"

# Check for CPU-only mode
if tf.config.list_physical_devices('GPU'):
    print("GPU is still available but forcing CPU for this task")
else:
    print("Running OCR on CPU (GPU disabled)")

# Specify the video file path
video_file_path = r"C:\Users\USER\Documents\TensorGo Assignment\TensorGoVideoTrain.mp4"
# Specify the output file path for storing CPU metrics
output_file_path_cpu = r"C:\Users\USER\Documents\TensorGo Assignment\cpu_ocr_analysis_results.txt"
...
with open(output_file_path_cpu, 'w') as f:  # Open file for writing results
    while cap.isOpened():
        ret, frame = cap.read()
        if not ret:
...
with open(output_file_path_cpu, 'a') as f:
    f.write("\n--- Exemplary Data Analysis (CPU Only) ---\n")
